...
class Gitlab:
    ...
    def change_default_branch(self,project_id):
        url = f"https://gitlab.com/api/v4/projects/{project_id}"
        parameters = {
        ...
        session = Session()
        session.headers.update(headers)
        response = session.put(url, params=parameters)
        data = json.loads(response.text)
        return data

    def demo(self,url):
        subprocess.run(['/home/gitlab/workspace/gitlab/script.sh', url])
        # subprocess.run(['/home/gitlab/workspace/api/gitlab-api/static/script.sh', url])
